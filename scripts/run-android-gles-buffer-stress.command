...
readonly LABEL="${1:-}"
readonly ROUNDS="${2:-8}"
readonly UPDATES="${3:-4000}"
readonly BUFFER_BYTES="${4:-16384}"
readonly SYNC_EVERY="${5:-120}"
readonly BARRIER_EVERY="${6:-64}"
readonly WARMUP_ROUNDS="${7:-$(( ROUNDS > 8 ? 7 : 1 ))}"
readonly ADB_SERVER_PORT="${TFT_ADB_SERVER_PORT:-5038}"
readonly SERIAL="${TFT_SERIAL:-emulator-5582}"
readonly EXPECTED_GRAPHICS_PROFILE="${TFT_GLES_STRESS_EXPECTED_GRAPHICS_PROFILE:-}"
readonly EXPECTED_ANGLE_ENABLED="${TFT_GLES_STRESS_EXPECTED_ANGLE_ENABLED:-}"
readonly EXPECTED_ANGLE_DISABLED="${TFT_GLES_STRESS_EXPECTED_ANGLE_DISABLED:-}"
readonly PROBE="${TFT_GLES_STRESS_PROBE:-$PROJECT_DIR/runtime/mactician-gles-buffer-stress.apk}"
readonly MEASUREMENT_ROOT="${TFT_GLES_STRESS_ROOT:-$PROJECT_DIR/runtime/measurements/android-gles-buffer-stress}"
readonly PACKAGE="${TFT_PACKAGE:-com.riotgames.league.teamfighttactics}"
readonly PROBE_PACKAGE="dev.sergeinaumov.mactician.glesprobe"
readonly PROBE_ACTIVITY="$PROBE_PACKAGE/.StressActivity"
readonly ADB="$(tft_resolve_adb)"
readonly JQ="${TFT_JQ:-$(command -v jq 2>/dev/null || true)}"

if [[ -z "$LABEL" || ! "$LABEL" =~ '^[A-Za-z0-9][A-Za-z0-9._-]*$' ]]; then
    print -u2 "Usage: ${0:t} <profile-label> [rounds updates bytes sync-every barrier-every warmup-rounds]"
    exit 2
fi
for numeric_name numeric_value minimum in \
        rounds "$ROUNDS" 2 \
        updates "$UPDATES" 1 \
        bytes "$BUFFER_BYTES" 1 \
        sync-every "$SYNC_EVERY" 1 \
        barrier-every "$BARRIER_EVERY" 0 \
        warmup-rounds "$WARMUP_ROUNDS" 1; do
    if [[ "$numeric_value" != <-> ]] || (( numeric_value < minimum )); then
        print -u2 "$numeric_name must be an integer of at least $minimum."
        exit 2
    fi
done
...
if [[ -z "$JQ" || ! -x "$JQ" ]]; then
    print -u2 "jq is required to validate and summarize GLES stress evidence."
    exit 1
fi
if [[ ! -f "$PROBE" ]]; then
    print -u2 "The Android GLES stress APK is unavailable: $PROBE"
    exit 1
fi
if [[ ! "$EXPECTED_GRAPHICS_PROFILE" =~ '^[a-z0-9][a-z0-9-]*$' ]]; then
    print -u2 "TFT_GLES_STRESS_EXPECTED_GRAPHICS_PROFILE is required."
    exit 2
fi
...
if ! "$ADB" -s "$SERIAL" get-state >/dev/null 2>&1; then
    print -u2 "The GLES stress probe requires an already running AVD on $SERIAL."
    exit 1
fi
...
if [[ "$ACTIVE_ANGLE_ENABLED" != "$EXPECTED_ANGLE_ENABLED" \
    ...
    print -u2 "Active ANGLE feature overrides do not match the requested candidate."
...
# Keep the synthetic sample isolated from the game and Android UI rendering.
...
"$ADB" -s "$SERIAL" shell am start -W \
    ...
    --ei rounds "$ROUNDS" \
    --ei updates "$UPDATES" \
    --ei bytes "$BUFFER_BYTES" \
    --ei sync_every "$SYNC_EVERY" \
    --ei barrier_every "$BARRIER_EVERY" \
...
if (( START_STATUS != 0 )) || ! grep -Fq 'Status: ok' "$RUN_DIR/start.txt"; then
    print -u2 "The Android GLES stress activity did not start: $RUN_DIR/start.txt"
    exit 1
fi

integer waited=0
while (( waited < 60 )); do
    "$ADB" -s "$SERIAL" logcat -d -v raw -s MacticianGLES:I '*:S' \
        2>/dev/null | tr -d '\r' > "$RAW_OUTPUT.next"
    mv -f "$RAW_OUTPUT.next" "$RAW_OUTPUT"
    if grep -F "$RUN_ID" "$RAW_OUTPUT" | grep -Fq '"kind":"buffer_stress_summary"'; then
        break
    fi
    if grep -F "$RUN_ID" "$RAW_OUTPUT" | grep -Fq '"kind":"failure"'; then
        print -u2 "The Android GLES stress activity reported a failure: $RAW_OUTPUT"
        exit 1
    fi
    sleep 1
    (( waited += 1 ))
...
if ! grep -Fq '"kind":"buffer_stress_summary"' "$ROUNDS_JSONL"; then
    print -u2 "The Android GLES stress activity timed out: $RAW_OUTPUT"
    exit 1
fi
if ! "$JQ" -e -s \
    ...
    print -u2 "The probe did not attest the guest ANGLE -> Vulkan renderer: $RAW_OUTPUT"
    exit 1
fi
if ! "$JQ" -e -s \
        --argjson rounds "$ROUNDS" \
        --argjson warmup_rounds "$WARMUP_ROUNDS" \
        '([.[] | select(.kind == "buffer_stress_round")] | length) == $rounds
         and ([.[] | select(.kind == "buffer_stress_summary")] | length) == 1
         and all(.[]; (.gl_error? // 0) == 0)
         and ([.[] | select(.kind == "buffer_stress_summary")][0].measured_rounds == ($rounds - $warmup_rounds))
         and ([.[] | select(.kind == "buffer_stress_round" and .warmup == true)] | length) == $warmup_rounds' \
        "$ROUNDS_JSONL" >/dev/null; then
    print -u2 "The probe output was incomplete or reported a GLES error: $RAW_OUTPUT"
    exit 1
fi
...
"$JQ" -s \
    ...
    --argjson bytes "$BUFFER_BYTES" \
    --argjson sync_every "$SYNC_EVERY" \
    --argjson barrier_every "$BARRIER_EVERY" \
    '{schema_version: 1, utc: $utc, "label": $profile_label, serial: $serial,
      ...
      workload: {rounds: $rounds, warmup_rounds: $warmup_rounds,
                 measured_rounds: ($rounds - $warmup_rounds), updates: $updates,
                 bytes: $bytes, sync_every: $sync_every, barrier_every: $barrier_every},
      attestation: ([.[] | select(.kind == "attestation")][0]),
      rounds: [.[] | select(.kind == "buffer_stress_round")],
      result: ([.[] | select(.kind == "buffer_stress_summary")][0])}' \
